...
The target reference must contain at least two independent venues.

Initial intended sources:
- Coinbase BTC-USD normalized through an observed Coinbase USDC-USD basis;
- Binance BTC-USDT normalized through an observed Binance USDC-USDT basis.

No USD=USDC or USDT=USDC parity assumption is permitted.

A reference observation is invalid if any required component is stale, malformed, future-dated, or if the independent normalized references disagree beyond the declared data-quality bound.

Reference source choice is frozen for Phase A unless an upstream venue/product becomes unavailable. Any replacement creates a new evidence cohort rather than being silently mixed into the original sample.

## 4. Clock semantics

Three clocks must remain distinct:

1. **Exchange/event timestamp** — retained only where the source documents what the timestamp represents.
2. **Local wall-clock receive timestamp** — used for comparable freshness/age checks.
3. **Local monotonic receive timestamp** — used for ordering and local latency/skew measurements.
...
Nanosecond timestamps must be retained as decimal strings or bigint until safely reduced; parsing them first as JavaScript numbers is invalid evidence.
...
- target local receive wall-clock and monotonic timestamps;
- precision-safe source timestamps where semantically valid;
...
- all reference inputs, conversions and receive timestamps;
- normalized cross-venue reference and dispersion;
- target/reference deviation in bps;
- market-status evidence when available;
- Bitvavo public trades around the episode with taker side and precision-safe timestamp;
...
Missing a horizon because data integrity is broken yields missing evidence, not carry-forward/interpolation.
...
Maker execution is PAPER-only and must use the existing conservative queue model:
...
- trade-through may count as strong fill evidence only with continuous trade-stream integrity and regular-trading evidence;
- trades outside the preregistered active quote window cannot fill the hypothetical order.

Phase A may evaluate a fixed set of descriptive active windows, but it must not convert the best retrospective window into a claim of edge. Any execution policy selected after Phase A is frozen before Phase B and tested OOS.
...
- whether conservative maker-fill evidence existed within each preregistered descriptive window;
- spread/depth/slippage at the observation time;
- reference dispersion and feed skew;
- adverse movement after any hypothetical fill evidence;
...
2. **Fill illusion:** apparent convergence exists but conservative maker-fill evidence is absent or concentrated only under permissive assumptions.
3. **Adverse selection:** hypothetical fills are systematically followed by continuation against the passive quote rather than reversion.
4. **Reference instability:** source dispersion/basis instability is large enough that the measured local dislocation cannot be distinguished reliably from reference noise.
5. **Timing illusion:** the effect disappears when alignment uses local receive/monotonic clocks rather than semantically invalid exchange timestamps.
...
- use API keys or private account data;
